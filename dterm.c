...
/*
 * Copyright 2007 Rumsey Stokes Ltd
...
rzsz(char *mode, char *file) {
	...
	/*
	 ...
	 Search the path; for each element look for xx and lxx
	 (where xx are the first two chars of mode).
	 */
	if(!pathenv) {
		pathenv = getenv("PATH");
		if(!pathenv) pathenv = "/usr/bin:/usr/local/bin";
	}
	for(p = pathenv; p; p = pe) {
		pe = strchr(p, ':');
		if(pe)	l = (pe++) - p;
		else	l = strlen(p);
		if(*p != '/') continue;
		snprintf(path, sizeof(path), "%.*s/%c%c",
				l, p, mode[0], mode[1]);
		if(!stat(path, &sb) && (sb.st_mode & S_IXUSR))
			break;
		snprintf(path, sizeof(path), "%.*s/l%c%c",
				l, p, mode[0], mode[1]);
		if(!stat(path, &sb) && (sb.st_mode & S_IXUSR))
			break;
	}
	if(!p) {
		fprintf(stderr, "%c%c not installed\n", mode[0], mode[1]);
		return -1;
	}
